# implementation of Spaceship - Chao Pan
import simplegui
import math
import random
...
class Sprite:
    ...
    def draw(self, canvas):
        animation_center = list(self.image_center)
        if self.animated:
            animation_center[0] = self.image_center[0] + (self.age % 24) * self.image_size[0] 
...
def draw(canvas):
    global time, started, lives, score, levels, rock_group
    ...
    explosion_info.get_center()
    explosion_info.get_size()
    
    # draw UI
    canvas.draw_text("Lives", [50, 50], 22, "White")
    canvas.draw_text("Score", [680, 50], 22, "White")
    canvas.draw_text('Levels', [370, 30],16,'Gray','monospace')
    canvas.draw_text(str(lives), [50, 80], 22, "White")
    canvas.draw_text(str(score), [680, 80], 22, "White")
    canvas.draw_text(str(levels), [370, 60], 16,'Gray','monospace')
    ...
    # Collide between rock and ship, lose 1 live
...
def rock_spawner():
    global rock_group, levels
    if started and len(rock_group) < 12:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * .6 * levels - .3, random.random() * .6 * levels - .3]
